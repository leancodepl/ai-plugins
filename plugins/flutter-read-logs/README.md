...
Claude reads the most recent run's logs and works your task against them. It is a context
loader, not an auto-analyzer.

## Included assets

- `skills/read-logs/SKILL.md`: the `/read-logs` workhorse. It resolves the per-project log,
...
## ⚠️ Security and data handling: read this first
...
(mask tokens/emails/keys before the model sees them). It is not built yet and is tracked as a possible
...
`/read-logs` derives, `/tmp/flutter-<repo>.log`).

## VS Code and Cursor (one-time, keeps F5)

Add to your `.vscode/settings.json` (gitignored in most projects, so it stays local.
That is correct, since the path is machine-specific):
...
## Zed or a terminal run task (one-time, clean transcript)
